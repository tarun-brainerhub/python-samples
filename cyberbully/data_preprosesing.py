...
#     model = pickle.load(open("model/finalized_model (1).sav", "rb"))
...
def custom_input_prediction(text):
    ...
    text = [text[0],]
    # to use this function we will need to define vectoriser first 
    vectoriser = pickle.load(open(r"model\vectoriser.sav", "rb"))
    text = vectoriser.transform(text)
    model = pickle.load(open(r"model\finalized_model.sav", "rb"))
    prediction = model.predict(text)
    prediction = prediction[0]
